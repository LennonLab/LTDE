...
df <- read.table("/Users/WRShoemaker/GitHub/LTDE/data/breseq/genetic_diversity_taxa.txt", 
...
df.merge.reorder <- select(df.merge, "FullSpecies", "n_muts", "mean_freq", "Theta", "Pi", "Tajimas_D") 

colnames(df.merge.reorder) <- c("$\\mathrm{Taxon}$", "$n_{muts}$", "$f$", 
                                "$\\theta_{W} \\, (\\mathrm{bp}^{-1})$", "$\\theta_{\\pi}\\, (\\mathrm{bp}^{-1})$", "$D_{T}$")
                                
mat <- xtable(df.merge.reorder, display = c("s", "s", "f","f", "g", "g", "g"), align=c("c","l","c","c", "c","c","c"), digits=c(0, 0, 0, 2, 2, 2, 2))
