...
colnames(df)[colnames(df)=="Taxon"] <- "Species"
...
colnames(df.merge.reorder) <- c("$\\mathrm{Taxon}$", "$\\Delta \\ell$", "$p_{BH}$")
                                
mat <- xtable(df.merge.reorder, display = c("s", "s", "f","g"), align=c("c","l","c","c"), digits=c(0, 0, 2, 2))
