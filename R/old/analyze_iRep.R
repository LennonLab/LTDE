...
df.irep <- read.table("data/iRep_clean.txt", 
                      header = TRUE, sep = "\t", row.names = 1, stringsAsFactors = FALSE)
colnames(df.irep)[1] <- "strain"


df.merge <- merge(df.weib, df.irep,by=c("rep","strain"))
df.merge <- df.merge[!(df.merge$strain=="KBS0812"),]

mixed.b0 <- lmer(log10(alpha) ~ iRep + (1|strain), data = df.merge)
summary(mixed.b0)
anova(mixed.b0)
...
plot(df.merge$iRep, log10(df.merge$alpha))

summary(lm(log10(df.merge$alpha) ~ df.merge$iRep))
