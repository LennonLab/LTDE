...
ggplot(data = rda.plot, aes(RDA1, RDA2)) +
  ...
  annotate("text", x=(rda.vecs[3,1]*scale.arrows) - 0.2, y=(rda.vecs[3,2]*scale.arrows)+0.3, label=TeX("$Yield$"), size = 4) +
  annotate("text", x=(rda.vecs[2,1]*scale.arrows) + 0.2, y=(rda.vecs[2,2]*scale.arrows)-0.2, label=TeX("$\\log_{10} (\\lambda) $"), size = 4) +
